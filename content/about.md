...
* Sven Peter. Sven has worked tirelessly on upstream Linux support fot Apple's Device Address Resolution Table (DART) required for USB support, PCIe support, Ethernet, Wi-Fi, and NVMe (internal storage) on the M1.

* Mark Kettenis, OpenBSD developer. Mark has written U-Boot drivers for the Apple M1 core peripherals, including the bringup needed for PCIe on Linux. Mark has also written OpenBSD drivers for the Apple M1 as a parallel effort to the Linux port.
